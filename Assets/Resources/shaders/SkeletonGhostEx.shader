...
Shader "Spine/SkeletonGhostEx" 
{
    Properties 
    {
		...
		_TextureColor("Texture Color",Range(0,1))=0
		//_TextureWrite("Texture Color",Range(0,))=0
    }
    SubShader 
    {
        ...
        Pass 
        {
            Tags {"LightMode" = "Always"}                      // This Pass tag is important or Unity may not give it the correct light information.
                ...
                fixed4 frag(vertex_output i) : COLOR
                {
					...
					if(_TextureColor>0)
						return tex*(1-_TextureColor)*i.color+_Color*tex.a*_TextureColor*i.color;
					else if(_TextureColor==0)
						return tex * (i.color * tex.a);
					else
						return tex*(1+_TextureColor)*i.color-(1,1,1,1)*_TextureColor*tex.a*i.color;
