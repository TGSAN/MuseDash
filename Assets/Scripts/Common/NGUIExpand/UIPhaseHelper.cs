...
public class UIPhaseHelper : MonoBehaviour
{
    ...
    //动态文字
    public bool isAnimateTxt = false;

    public float txtAnimTime;
    public AnimationCurve txtCurve;
    ...
    private void __NotifyForText(string key, object value)
    {
        ...
        else
        {
            var startNum = 0;
            var endNum = 0;
            if (int.TryParse(this._label.text, out startNum) && int.TryParse(value.ToString(), out endNum))
            {
                DOTween.To(() => startNum, to =>
                {
                    startNum = to;
                    this._label.text = startNum.ToString();
                }, endNum, txtAnimTime).SetEase(txtCurve);
            }
        }
    }
