...
namespace FormulaBase
{
    public class PetManageComponent : CustomComponentBase
    {
        ...
        private List<FormulaHost> m_ListEquipedPetHosts = new List<FormulaHost>();
        ...
        private void PetLevelUpCallback(bool _success)
        {
            if (_success)
            {
        ...
        public FormulaHost CreateItem(int idx)
        {
            ...
            var typeName = ConfigPool.Instance.GetConfigValue("items", idx.ToString())["type"].ToString();
            if (typeName != "servant")
            {
            ...
            host = FomulaHostManager.Instance.CreateHost(HOST_IDX);
            if (host != null)
            {
        ...
        private List<RewardData> m_AllPet = new List<RewardData>();
