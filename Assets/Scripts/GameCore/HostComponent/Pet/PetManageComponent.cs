	...
	public class PetManageComponent : CustomComponentBase {
		...
		public  FormulaHost HaveTheSameID(int _ID) {
			...
			for (int i = 0, max = templist.Count; i < max; i++) {
				FormulaHost host = templist [i];
				if (host == null) {
					continue;
				}

				targetId = host.GetDynamicIntByKey (SignKeys.ID);
				if (_ID == targetId) {
					if (host.GetDynamicIntByKey (SignKeys.STACK_NUMBER) != 1) {//可以堆叠
						int stackNumber = host.GetDynamicIntByKey (SignKeys.STACKITEMNUMBER);
						stackNumber++;
						host.SetDynamicData (SignKeys.STACKITEMNUMBER, stackNumber);
						return templist [i];
					}
				}
			}
		...
		public void GetAllEquipedPet(ref int _hp,ref int _df,ref int _att,ref int _crit) {
			for (int i = 0; i < m_ListEquipedPetHosts.Count; i++) {
				FormulaHost host = m_ListEquipedPetHosts [i];
				if (host == null) {
					continue;
				}

				if (host.GetDynamicIntByKey (SignKeys.EQUIPEDQUEUE) / 10 == RoleManageComponent.Instance.GetFightGirlIndex ()) {
					_hp += (int)host.Result (FormulaKeys.FORMULA_137);					//Hp
					_att += (int)host.Result (FormulaKeys.FORMULA_139);					//Def
					_df += (int)host.Result (FormulaKeys.FORMULA_141);	//Att
					_crit += (int)host.Result (FormulaKeys.FORMULA_143);					//Crt
				}
			}
		}
	}
