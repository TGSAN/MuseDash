...
using Assets.Scripts.Tool.Commons.Editor;
using HutongGames.PlayMaker.Actions;
using UnityEditor;
using UnityEngine;

namespace Assets.Scripts.Tool.FormulaTreeManager.Editor
{
    [Serializable]
    ...
    public class FormulaTreeEditor : SingtonEditor<FormulaTreeEditor>
    {
        ...
        public static GUISkin skin
        {
            get
            {
                if (m_Skin == null)
                {
                    ...
                    {
                        m_Skin =
                            AssetDatabase.LoadAssetAtPath(StringCommon.PRFormulaTreeEditorSkinPathInAsset, typeof(GUISkin))
                                as GUISkin;
                    }
                }
                return m_Skin;
        ...
        public static void CreateFormulaData()
        {
            ...
            StringCommon.FormulaTreePathInResources = string.IsNullOrEmpty(path) ? StringCommon.FormulaTreePathInResources : path;
            AssetDatabase.CreateAsset(formulaTree, StringCommon.ResourcesPathInAssets + StringCommon.FormulaTreePathInResources + ".asset");
        }

        private void OnEnable()
        {
            var res = Resources.Load<FormulaTree>(StringCommon.FormulaTreePathInResources);
            if (res != null)
            {
        ...
        private void OnPathEditWindow()
        {
            ...
            {
                StringCommon.FormulaTreePathInResources = GUILayout.TextField(StringCommon.FormulaTreePathInResources);
                if (GUILayout.Button("Setup"))
                {
                    CreateFormulaData();
                    OnEnable();
                }
                EditorPrefs.SetString("FormulaTree Path", StringCommon.FormulaTreePathInResources);
            }, "Path Edit");
        }
