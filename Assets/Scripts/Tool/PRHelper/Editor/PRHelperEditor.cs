...
namespace Assets.Scripts.Tool.PRHelper.Editor
{
    ...
    public class PRHelperEditorNode : EditorNode
    {
        ...
        public void AddChildEvent(EditorNode childNode, List<EditorNode> allEditorNodes, PREvents.EventType eventType)
        {
            ...
            if (n == null) return;
            var unityEvent = PRHelper.OnEvent(node, eventType);
            var func = Delegate.CreateDelegate(typeof(UnityAction<string>), PRHelperWindow.prHelper, typeof(PRHelper).GetMethod("Play")) as UnityAction<string>;
    ...
    public class PRHelperWindow : EditorWindow
    {
        ...
        private static PRHelper m_PRHelper;
        ...
        public static GUISkin skin
        {
            get
            {
                if (m_Skin == null)
                {
                    if (EditorGUIUtility.isProSkin)
                    {
                        m_Skin = AssetDatabase.LoadAssetAtPath(StringCommon.PRFormulaTreeEditorSkinPathInAsset, typeof(GUISkin)) as GUISkin;
                    }
                }
                return m_Skin;
            }
        ...
        public static PRHelper prHelper
        {
            get
            {
                var selectedGO = Selection.activeGameObject;
                if (selectedGO == null) return m_PRHelper;
                m_PRHelper = selectedGO.GetComponent<Tool.PRHelper.PRHelper>();
                return m_PRHelper;
            }
        }
                ...
                if (m_VMWndRect.Contains(mousePos))
                {
                    ...
                    foreach (var funcContent in m_VMContents)
                    {
                        ...
                        menu.AddItem(funcContent, false, obj =>
                        {
                            ...
                            PRHelper.OnEvent(node, eventType);
