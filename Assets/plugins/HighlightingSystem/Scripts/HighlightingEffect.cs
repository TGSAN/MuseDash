...
public class HighlightingEffect : MonoBehaviour
{
	...
	private void InitShareCamera() {
		if (!this.shaderCameraGO) {
			this.shaderCameraGO = new GameObject("HighlightingCamera", typeof(Camera));
			this.shaderCameraGO.GetComponent<Camera>().enabled = false;
			this.shaderCameraGO.hideFlags = HideFlags.HideAndDontSave;
		}
		
		if (!this.shaderCamera) {
			this.shaderCamera = shaderCameraGO.GetComponent<Camera>();
			this.shaderCamera.CopyFrom(refCam);
			...
			this.shaderCamera.cullingMask = layerMask;
			this.shaderCamera.rect = new Rect(0f, 0f, 1f, 1f);
			this.shaderCamera.renderingPath = RenderingPath.VertexLit;
			this.shaderCamera.hdr = false;
			this.shaderCamera.useOcclusionCulling = false;
			this.shaderCamera.backgroundColor = new Color(0f, 0f, 0f, 0f);
			this.shaderCamera.clearFlags = CameraClearFlags.SolidColor;
		}
	}
	...
	void OnPreRender()
	{
		...
		stencilBuffer = RenderTexture.GetTemporary((int)this.renderCamera.pixelWidth, (int)this.renderCamera.pixelHeight, stencilZBufferDepth, RenderTextureFormat.ARGB32);

		this.InitShareCamera ();
